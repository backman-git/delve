...
import (
	...
	"github.com/backman-git/delve/pkg/proc/amd64util"
)
...
func ptraceGetRegset(tid int) (regset amd64util.AMD64Xstate, err error) {
	...
	if err != syscall.Errno(0) {
		if err == syscall.ENODEV || err == syscall.EIO || err == syscall.EINVAL {
			// ignore ENODEV, it just means this CPU or kernel doesn't support XSTATE, see https://github.com/backman-git/delve/issues/1022
